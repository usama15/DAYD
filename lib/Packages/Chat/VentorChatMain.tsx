...
export default function VentorChatMain(props: any) {
  ...
  const User = async (user: any) => {
    ...
    await firestore()
      ...
      .onSnapshot(snapshot => {
        ...
        setMessages(newPost.filter(x => x.ventorEmail == user.email));
      });
  };
